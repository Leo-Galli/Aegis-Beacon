...
const toMorse=(s)=>{return s.toUpperCase().split('').map(c=>MORSE[c]||' ').join('  ');};
...
window.addFreq=()=>{const v=parseFloat(document.getElementById('newFreq').value);if(isNaN(v)||v<410||v>525||freqs.length>=10)return;freqs.push(v);document.getElementById('newFreq').value='';renderFreqs();};
window.delFreq=(i)=>{if(freqs.length<=1)return;freqs.splice(i,1);renderFreqs();};
renderFreqs();

// ── Knob visual (decorative) ──────────────────────────────────────────────────
window.updateKnob=(markerId, valId, rawVal, min, max, suffix)=>{
