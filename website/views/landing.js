 ...
 *
 * Stunning hero with animated gradient background, glass-morphism
 * feature cards, proper stats bar, and call-to-action.
 */
...
const FEATURES = [
  { tag: 'LORA RF', title: 'Long-Range Communication', desc: 'SX1262-based transceiver with +30 dBm PA for emergency beacon transmissions up to 15 km line-of-sight at 410-525 MHz.' },
  { tag: 'EFFICIENCY', title: 'Ultra-Low Power', desc: 'Deep sleep at 10 microamps with 30-second hardware watchdog. WiFi/BT disabled in beacon mode to save 120 mA continuous draw.' },
  { tag: 'INFRASTRUCTURE', title: 'Zero Infrastructure', desc: 'No GSM, WiFi, or commercial satellite requirements. Direct peer-to-peer radio communication at 410-525 MHz.' },
  { tag: 'OPEN SOURCE', title: 'MIT Licensed', desc: 'Full source code, hardware schematics, and BOM under MIT license. Community-driven development with PlatformIO.' },
  { tag: 'SAFETY', title: 'Emergency Certified', desc: 'CW Morse beacon with GPS coordinates for SAR operations. Compliant with PMR446 and GMRS wilderness protocols.' },
  { tag: 'CONFIGURATION', title: 'WiFi Config Portal', desc: 'Captive portal at 192.168.4.1 for field configuration. Adjust frequencies, WPM, power output, and GPS settings.' }
];
...
function renderHero() {
  const statBar = STATS.map((s) => `
    <div class="text-center">
      <div class="text-xl sm:text-2xl font-bold font-mono text-white">${s.value}</div>
      <div class="text-[9px] sm:text-[10px] font-mono text-orange-200/70 uppercase tracking-widest mt-1">${s.label}</div>
    </div>`).join('');

  return `<section class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-orange-600 via-orange-500 to-amber-500 p-[1px]">
    <div class="relative rounded-3xl bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#0f172a] overflow-hidden">
      <!-- Animated background orbs -->
      <div class="absolute inset-0 overflow-hidden pointer-events-none">
        <div class="absolute -top-32 -right-32 w-96 h-96 bg-orange-500/10 rounded-full blur-[100px] animate-pulse"></div>
        <div class="absolute -bottom-32 -left-32 w-96 h-96 bg-amber-500/10 rounded-full blur-[100px] animate-pulse" style="animation-delay:2s"></div>
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-orange-600/5 rounded-full blur-[120px]"></div>
      </div>
      ...
      <div class="relative z-10 px-6 sm:px-12 py-16 sm:py-20 text-center space-y-8">
        ...
        <h1 class="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight text-white leading-[1.1]">
          <span class="notranslate">Aegis</span><span class="text-orange-400">-</span><span class="notranslate">Beacon</span>
...
function renderFeatures() {
  const colorMap = {
    LORA: { accent: 'bg-orange-500', text: 'text-orange-400' },
    EFFICIENCY: { accent: 'bg-emerald-500', text: 'text-emerald-400' },
    INFRASTRUCTURE: { accent: 'bg-sky-500', text: 'text-sky-400' },
    OPEN: { accent: 'bg-violet-500', text: 'text-violet-400' },
    SAFETY: { accent: 'bg-rose-500', text: 'text-rose-400' },
    CONFIGURATION: { accent: 'bg-amber-500', text: 'text-amber-400' }
  };
  const cards = FEATURES.map((f) => {
    const c = colorMap[f.tag.split(' ')[0]] || colorMap.LORA;
    return `
    <div class="group relative bg-white dark:bg-[#0f1626] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-3 hover:border-orange-500/40 hover:shadow-xl hover:shadow-orange-500/5 transition-all duration-300 hover:-translate-y-1">
      <div class="w-10 h-10 rounded-xl ${c.accent}/10 flex items-center justify-center">
        <div class="w-2.5 h-2.5 rounded-full ${c.accent}"></div>
      </div>
  ...
  return `<section class="space-y-10">
    <div class="text-center space-y-3">
      ...
      <h2 class="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white">Built for Critical Missions</h2>
      <p class="text-slate-500 dark:text-slate-400 max-w-xl mx-auto">Every component selected for reliability when lives depend on it.</p>
    </div>
...
function renderModes() {
  ...
  const cards = MODES.map((m) => {
    ...
    return `
    <div class="border ${c.border} ${c.bg} rounded-2xl p-6 space-y-4 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
  ...
  return `<section class="space-y-10">
    <div class="text-center space-y-3">
      ...
      <h2 class="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white">Four Modes for Every Scenario</h2>
      <p class="text-slate-500 dark:text-slate-400 max-w-xl mx-auto">From passive beacon to emergency high-power output.</p>
    </div>
...
function renderHardware() {
  ...
  return `<section class="space-y-10">
    <div class="text-center space-y-3">
      <span class="text-[10px] font-mono font-bold text-orange-600 dark:text-orange-400 uppercase tracking-widest">// HARDWARE</span>
      <h2 class="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white">Bill of Materials</h2>
      <p class="text-slate-500 dark:text-slate-400 max-w-xl mx-auto">Every component listed with sourcing links and pricing.</p>
    </div>
    <div class="max-w-3xl mx-auto bg-white dark:bg-[#0f1626] border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
      ${rows}
      <div class="flex items-center justify-between pt-4 mt-2 border-t-2 border-orange-200 dark:border-orange-900/40">
        ...
        <span class="text-lg font-mono font-bold text-orange-600 dark:text-orange-400">~$23-28 USD</span>
      </div>
      <div class="mt-4">
        <a href="/builder" class="inline-flex items-center gap-2 px-5 py-2.5 bg-orange-500 hover:bg-orange-400 text-white font-mono text-xs font-bold rounded-lg transition-all shadow-md shadow-orange-500/20 hover:shadow-orange-400/30">
          Open BOM Builder
...
function renderCTA() {
  return `<section class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-orange-600 via-orange-500 to-amber-500 p-[1px]">
    <div class="relative rounded-3xl bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#0f172a] p-10 sm:p-14 text-center space-y-6">
      <div class="absolute inset-0 overflow-hidden pointer-events-none">
        <div class="absolute -top-20 -right-20 w-60 h-60 bg-orange-500/10 rounded-full blur-[80px]"></div>
        <div class="absolute -bottom-20 -left-20 w-60 h-60 bg-amber-500/10 rounded-full blur-[80px]"></div>
      </div>
      <div class="relative z-10 space-y-6">
        <h2 class="text-3xl sm:text-4xl font-bold text-white">Ready to Build Your Own?</h2>
        <p class="text-slate-300 max-w-xl mx-auto">Complete documentation, step-by-step assembly guide, interactive firmware demo, and BOM calculator. Everything you need.</p>
