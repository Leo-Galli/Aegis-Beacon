...
- A specific failing PR check, missing workflow log, or CI failure triage
...
- Treating repo-specific CI debugging as a documentation question
- Letting adjacent domains absorb the request when a sharper fit exists

## Security Best Practices (from GitHub docs)

- Set `permissions:` explicitly at the workflow or job level; default to the least privilege needed (usually `contents: read`).
- Never interpolate `${{ secrets.X }}` directly into `run:` shell lines; pass secrets through the `env:` mapping instead.
- Avoid `pull_request_target` with a checkout of the PR's ref unless the workflow is trusted; it exposes `GITHUB_TOKEN` to untrusted code.
- Prefer OIDC (`permissions: id-token: write`) over long-lived cloud credentials for deployments.
- Pin third-party actions to a full-length commit SHA for supply-chain security where the policy demands it.
- Use `actions/cache` with a `key` derived from `hashFiles(...)` so cache invalidation is automatic.